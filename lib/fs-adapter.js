...
class FSAdapter {
    ...
    async append(options) {
        ...
        const path = pathLib.join(this._baseDirectory, options.path);
        const fd = await fs.open(path, 'a');
        await fs.appendFile(fd, options.data);
        await fs.close(fd);
        return { path: pathLib.join(parsePath.directoryName, parsePath.fileName) };
    }

    async appendMultiParts(options) {
        const parsePath = this._parsePath(options.path);
        const path = pathLib.join(this._baseDirectory, options.path);
        const fd = await fs.open(path, 'a');
        for (const d of options.data) { // eslint-disable-line
